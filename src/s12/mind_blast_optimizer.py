...
def get_mind_blast_base_damage_mapping() -> dict[int, float]:
    ...
    for pair in itertools.pairwise(range(1, 61)):
        prev = level_to_avg_dmg[pair[0]]
        nxt = level_to_avg_dmg[pair[1]]
        inc_percent = 1.0 * (nxt - prev) / prev * 100
        print(f"Level {pair[0]} to {pair[1]}: {inc_percent}%")
