# Season 13 Porting Status (from Season 12)

This file tracks the status of porting changes from the Season 12 branch to the Season 13 branch.

## Status Legend
- [ ] Pending
- [x] Ported

## Commits to Port

- [ ] **89091ac** - Show HP5 and MP5 in dungeons
- [ ] **ea9333a** - Hide 0ed 0os eth Infinity bases
- [ ] **5a0d7c6** - Hide rare chests; show magic amulets to craft for PvP tournament
- [ ] **abe381e** - Show Infinity bases if SOC=4 or SOC=0 & EDAM>0
- [ ] **f94e894** - Hide 4os/5os Flail and show Phase Blades again
- [ ] **7255ce3** - Hide 5os Heavy Crossbow base
- [ ] **735a8fd** - Show superior 4os Demon Crossbow for Wrath base
- [ ] **ba71342** - Show 5os Heavy Crossbow for offhand Call to Arms base
- [ ] **61abda9** - Remove high drop sound from Death's Hand, Guard, and Tancred's Hobnails
- [ ] **e0d7439** - Unid rare Necromancer heads now only show for FILTLVL<7
- [ ] **ca290a5** - Show Death's Hand, Death's Guard, and Tancred's Hobnails for FILTLVL<8
- [ ] **70406d4** - Hide arrows & bolts for non-Amazon
- [ ] **b3d92e9** - Hide Arcanna's Flesh Light Plate
- [ ] **ec4cb25** - Hide non-15ed superior Faith bases
- [ ] **4d81b5a** - Hide magic amulets
- [ ] **92e359b** - Hide unid magic Necromancer heads
- [ ] **f6fa83d** - Hide unid magic amulets and Phase Blade for Charsi
- [ ] **5e5fdf1** - Update low rune and craft rune decorators
- [ ] **c2632a3** - Hide HP5
- [ ] **e4ec6a4** - Show 4os Faith Crossbow base with ED>0 for FILTLVL<8
- [ ] **44f8242** - Hide magic Amulets
- [ ] **8cfd775** - Hide Charsi's Phase Blades
- [ ] **8227097** - Update RUNE_LOW_CRAFT prefix/suffix to more closely match season 11
- [ ] **cecd7a5** - Update README to explicitly call out the original repo
- [ ] **a882ebf** - Show crafting low runes Tir, Eth, Ort, Ral, Amn, Sol for FILTLVL<8
- [ ] **6c43e99** - Remove map icon for magic jewels FILTLVL 7+
- [ ] **0482c06** - Remove map marker for Infused Arcane Orb
- [ ] **0189aac** - Show unid magic Tiara or Diadem for FILTLVL 7
- [ ] **b0d2ed6** - Show magic unid jewels bigger for FILTLVL 7
- [ ] **550322e** - Show HP5 for FILTLVL 7+.  Show unid rare boots, belt, and chest for FILTLVL<8
- [ ] **7f566bc** - Hide unidentified rare scepters and staves for FILTLVL 7+
- [ ] **1854ee2** - Show non-eth phase blade and magic unid amulet for Charsi
- [ ] **2b86ec3** - Show unid magic jewels for FILTLVL 8
- [ ] **347be73** - Unid magic tiara and diadem only show for FILTLVL<7
- [ ] **4d99c43** - Show unid rare jewelry for FILTLVL<8
- [ ] **9357062** - Only show unid magic phase blade for FILTLVL 8
- [ ] **715291a** - Initial Season 12 commit
